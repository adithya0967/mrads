...
export default function SolutionsSection() {
  return (
    <section className="relative border-b border-line bg-[#080808]" id="solutions">
      <div className="container-site py-20 md:py-28">
        <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
          ...
          <div className="max-w-sm">
            ...
            <Link
              href="/solutions"
              className="mt-3 inline-flex items-center gap-1.5 text-[13px] font-semibold text-brand hover:text-brand-hover transition-colors"
            >
        ...
        <div className="mt-14 grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {solutions.map((item) => (
            <article
              key={item.title}
              className={`group flex flex-col rounded-2xl border border-white/[0.08] bg-[#0D0D0D] overflow-hidden hover:border-brand/40 hover:shadow-[0_15px_40px_rgba(0,0,0,0.8),0_0_20px_rgba(216,31,66,0.15)] transition-all duration-300 ${
                item.featured ? 'md:col-span-2 lg:col-span-2' : ''
              }`}
            >
              <Link href={item.to} className="block flex-1 flex flex-col">
                <div
                  className={`img-zoom relative overflow-hidden bg-black ${
                ...
                >
                  ...
                  <div className="absolute top-4 left-4 right-4 flex items-center justify-between">
                    ...
                    <span className="px-2.5 py-0.5 rounded-full bg-brand/90 text-white text-[10px] font-bold uppercase tracking-wider shadow-sm">
                      {item.tag}
                    </span>
                  </div>
                </div>
                ...
                <div className="p-6 sm:p-7 flex-1 flex flex-col justify-between">
                  ...
                  <div className="mt-6 pt-4 border-t border-white/[0.06] flex items-center justify-between">
                    ...
                    <span className="flex items-center justify-center w-7 h-7 rounded-full bg-white/[0.05] group-hover:bg-brand text-paper transition-all">
                      <ArrowRight size={13} />
                    </span>
                  </div>
                </div>
